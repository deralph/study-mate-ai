...
function getMaterialContext(userId: string, materialIds: string[]): { context: string; refs: string[] } {
...
async function generateReply(userId: string, content: string, materialIds: string[]) {
  const { context, refs } = getMaterialContext(userId, materialIds);
...
chatRouter.get('/sessions', (req: AuthedRequest, res) => {
  const rows: any[] = db.prepare(`
...
chatRouter.post('/sessions', (req: AuthedRequest, res) => {
  const materialIds: string[] = Array.isArray(req.body?.materialIds) ? req.body.materialIds : [];
  const id = uid();
  db.prepare('INSERT INTO chat_sessions (id, user_id, title, material_ids) VALUES (?,?,?,?)')
    .run(id, req.userId!, 'New chat', JSON.stringify(materialIds));
  res.json({ session: { id, title: 'New chat' } });
});

chatRouter.get('/sessions/:id/messages', (req: AuthedRequest, res) => {
  const session: any = db.prepare('SELECT * FROM chat_sessions WHERE id=? AND user_id=?').get(req.params.id, req.userId!);
  if (!session) return res.status(404).json({ error: 'Not found' });
  const messages: any[] = db.prepare('SELECT * FROM chat_messages WHERE session_id=? ORDER BY timestamp ASC').all(req.params.id);
  res.json({
...
chatRouter.post('/sessions/:id/messages', async (req: AuthedRequest, res) => {
  ...
  const userId = uid();
  const ts = nowIso();
  db.prepare('INSERT INTO chat_messages (id, session_id, role, content, timestamp) VALUES (?,?,?,?,?)')
    .run(userId, session.id, 'user', content, ts);

  const reply = await generateReply(req.userId!, content, materialIds);
  const aiId = uid();
  const aiTs = nowIso();
  db.prepare('INSERT INTO chat_messages (id, session_id, role, content, references_json, timestamp) VALUES (?,?,?,?,?,?)')
  ...
  if (session.title === 'New chat') {
    ...
    db.prepare('UPDATE chat_sessions SET updated_at=? WHERE id=?').run(aiTs, session.id);
  }
...
chatRouter.delete('/sessions/:id', (req: AuthedRequest, res) => {
