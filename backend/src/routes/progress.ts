...
progressRouter.get('/stats', async (req: AuthedRequest, res) => {
  const userId = req.userId!;
  const user: any = await db.prepare('SELECT study_streak, level, points, department FROM users WHERE id=?').get(userId);
  const totals: any = await db.prepare(`SELECT COALESCE(SUM(duration_minutes),0) AS minutes FROM study_sessions WHERE user_id=?`).get(userId);
  const materialCount: any = await db.prepare('SELECT COUNT(*) AS c FROM materials WHERE user_id=?').get(userId);
  const quizCount: any = await db.prepare('SELECT COUNT(*) AS c FROM quiz_attempts WHERE user_id=?').get(userId);
  const avg: any = await db.prepare('SELECT COALESCE(AVG(percentage),0) AS a FROM quiz_attempts WHERE user_id=?').get(userId);
  const completedRecs: any = await db.prepare('SELECT COUNT(*) AS c FROM recommendations WHERE user_id=? AND completed=1').get(userId);
  const totalRecs: any = await db.prepare('SELECT COUNT(*) AS c FROM recommendations WHERE user_id=?').get(userId);
  const activeDays: any = await db.prepare(`SELECT COUNT(DISTINCT date(created_at)) AS c FROM study_sessions WHERE user_id=? AND date(created_at) >= date('now','-13 days')`).get(userId);
  const firstLast: any = await db.prepare(`SELECT MIN(percentage) AS first_score, MAX(completed_at) AS last_time FROM quiz_attempts WHERE user_id=?`).get(userId);
  const latest: any = firstLast.last_time ? await db.prepare('SELECT percentage AS last_score FROM quiz_attempts WHERE user_id=? AND completed_at=? LIMIT 1').get(userId, firstLast.last_time) : { last_score: 0 };

  const studyBySubject: any[] = await db.prepare(`
  ...
  const weeklyPerformance: any[] = await db.prepare(`
  ...
  const subjectBreakdown: any[] = await db.prepare(`
    SELECT s.subject,
  ...
  const recentActivity: any[] = await db.prepare(`
  ...
  const upcomingReminders: any[] = await db.prepare('SELECT title, time, recurrence FROM reminders WHERE user_id=? AND enabled=1 ORDER BY created_at DESC LIMIT 5').all(userId);
  const latestMaterials: any[] = await db.prepare('SELECT title, subject, file_type, upload_date FROM materials WHERE user_id=? ORDER BY upload_date DESC LIMIT 5').all(userId);
...
progressRouter.post('/session', async (req: AuthedRequest, res) => {
  ...
  const user: any = await db.prepare('SELECT study_streak, last_active_date FROM users WHERE id=?').get(req.userId!);
  ...
  await db.prepare('UPDATE users SET study_streak=?, last_active_date=?, points=points+5 WHERE id=?')
